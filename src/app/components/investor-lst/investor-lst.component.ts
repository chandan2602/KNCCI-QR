...
@Component({
  selector: 'app-investor-lst',
  templateUrl: './investor-lst.component.html',
  styleUrls: ['./investor-lst.component.css']
})
export class InvestorLstComponent extends BaseComponent implements OnInit {
  parentMessage: any = ''; parentUser: any = 0;
  roleId: any = ''; listGrid: any[] = []; isAddEdt = false; approve_comments: any = ''; isAprveRejct: any = 2;
  tableId: any = 0; rating: any = '1'; screnType: string = 'add';
  ...
  LoadGrid() { //InternshipJobs/GetInvestorStartUpList/{user_id} 
    this.listGrid = [];
    // if(this.roleId != '' && this.roleId != null) {
      this.CommonService.activateSpinner();
      this.CommonService.getCall(`InternshipJobs/GetInvestorStartUpList/${sessionStorage.UserId}`, '', false).subscribe(
        (res: any) => {
          if(res?.status == true) {
  ...
  onCheck(evnt: any) {
    ...
    if(this.screnType == 'view') {
      this.rating = this.tableId?.investorDetails[0]?.rating,
      this.approve_comments = this.tableId?.investorDetails[0]?.comments
    }
  ...
  Save() {// ApprovedOrRejected
    ...
    let payLoad: any = {id: this.tableId?.startup_id, status_id: this.isAprveRejct, 
      user_id: this.tableId?.investorDetails[0]?.startup_investor_id, // `${sessionStorage.UserId}`, //  this.tableId?.incubatorDetails[0]?.startup_investor_id, 
      role_id: 8, comments: this.approve_comments,
      approvel_type: 'Investor', rating: this.rating, sector_id: this.tableId?.sector_id ?? 0}
