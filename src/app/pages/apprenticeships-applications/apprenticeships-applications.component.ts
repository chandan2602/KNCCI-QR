...
export class ApprenticeshipsApplicationsComponent implements OnInit {
  ...
  loadApplications() {
    this.isLoading = true;
    // Use the dedicated applications endpoint that filters enrolled students
    this.commonService.getApprenticeshipApplications().subscribe(
      (res: any) => {
        if (res?.status === true && res?.data) {
          // Only show applications where students have actually enrolled
